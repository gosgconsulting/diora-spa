...
interface FindUsSectionProps {
  gmapSection?: SchemaItem;
  contactInfoSection?: SchemaItem;
}
...
// Render contact info item (Address, Phone, Email)
const ContactInfoItem = ({ items, icon: Icon }: { items: SchemaItem[]; icon: any }) => {
  const headingItem = getItemsByType(items, 'heading')[0];
  const textItem = getItemsByType(items, 'text')[0];
  const linkItem = getItemsByType(items, 'link')[0];

  const heading = headingItem?.content || "";
  ...
  return (
    <div className="flex items-start space-x-4">
      <div className="flex-shrink-0 w-6 h-6 mt-1">
        <Icon className="w-6 h-6" style={{ color: '#3a2c1b' }} />
      </div>
      <div>
      ...
      <div className="absolute top-32 right-8 z-10 w-96 max-w-md">
        <div className="bg-white rounded-2xl p-8 shadow-2xl">
          ...
          <div className="space-y-6">
            {/* Address */}
            {addressInfo && (
              <ContactInfoItem items={addressInfo.items || []} icon={MapPin} />
            )}

            {/* Phone */}
            {phoneInfo && (
              <ContactInfoItem items={phoneInfo.items || []} icon={Phone} />
            )}

            {/* Email */}
            {emailInfo && (
              <ContactInfoItem items={emailInfo.items || []} icon={Mail} />
            )}
            ...
            {chatButton && (
              <div className="pt-4">
                ...
                >
                  ...
                  <span>{chatButton.content || "Open Chat"}</span>
                </Button>
              </div>
            )}
